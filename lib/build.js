...
async function build(gyp, argv) {
  ...
  async function loadConfigGypi() {
    let data
    try {
      const configPath = path.resolve(gyp.outputDir, 'config.gypi')
  ...
  async function findSolutionFile() {
    const files = await glob(`${gyp.outputDir}/*.sln`)
    if (files.length === 0) {
      if (
        gracefulFs.existsSync(path.join(gyp.outputDir, 'Makefile')) ||
        (await glob(`${gyp.outputDir}/*.mk`)).length !== 0
      ) {
        command = makeCommand
        await doWhich(false)
        return
  ...
  async function doBuild(msvs) {
    ...
    if (msvs) {
      ...
      const p = archLower === 'x64' ? 'x64' : archLower === 'arm' ? 'ARM' : archLower === 'arm64' ? 'ARM64' : 'Win32'
    ...
    if (!win) {
      ...
      buildBinsDir = path.resolve(gyp.outputDir, 'node_gyp_bins')
