...
async function createConfigGypi({ gyp, buildDir, nodeDir, vsInfo, python }) {
  ...
  log.verbose(`${gyp.outputDir}/` + configFilename, 'creating config file')
  ...
  log.silly(`${gyp.outputDir}/` + configFilename, config)

  // now write out the config.gypi file to the build/ dir
  const prefix = '# Do not edit. File was generated by node-gyp\'s "configure" step'

  const json = JSON.stringify(config, boolsToString, 2)
  log.verbose(`${gyp.outputDir}/` + configFilename, 'writing out config file: %s', configPath)
