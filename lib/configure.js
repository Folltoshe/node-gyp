...
async function configure(gyp, argv) {
  ...
  async function getNodeDir() {
    ...
    if (!gyp.opts.nodedir && process.config.variables.use_prefix_to_find_headers) {
      ...
      let availVersion
      try {
        const nodeVersionH = readFileSync(path.join(prefix, 'include', 'node', 'node_version.h'), { encoding: 'utf8' })
      ...
      if (availVersion === release.version) {
        ...
        log.verbose('using local node headers based on prefix', 'setting nodedir to ' + gyp.opts.nodedir)
      }
    }
